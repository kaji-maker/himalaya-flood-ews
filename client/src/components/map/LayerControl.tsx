...
export const LayerControl: React.FC<LayerControlProps> = ({ layers, onToggleLayer }) => {
  const layerButtons: { key: keyof MapLayerState; label: string; icon: React.ReactNode; desc: string }[] = [
    ...
    },
    {
      ...
      icon: <Zap className="w-4 h-4 text-amber-400" />,
      desc: 'M4.5+ Epicenters & Moraine PGA Contours',
    },
    {
    ...
    },
    {
      ...
      label: 'InSAR Moraine Creep (SBAS)',
      icon: <Activity className="w-4 h-4 text-emerald-400" />,
      desc: 'Sentinel-1 Moraine Subsidence Hotspots',
    },
    {
      key: 'cueSlewFootprint',
      label: 'Cue-and-Slew Tasking',
      icon: <Target className="w-4 h-4 text-indigo-400" />,
      desc: 'Targeted SkySat/WorldView Footprints',
    },
    {
      key: 'edgeSensors',
      label: 'Gorge Sensors & SCADA',
      icon: <Cpu className="w-4 h-4 text-amber-400" />,
      desc: 'Riverbed Geophones & Dam Spillway Gates',
    },
    {
      key: 'mndwiWater',
