...
class LakeVectorizer:
    ...
    TARGET_UTM_EPSG = 32645       # UTM Zone 45N (Central / Eastern Himalaya >= 84°E)
    TARGET_UTM_WEST_EPSG = 32644  # UTM Zone 44N (Western Himalaya < 84°E)
    ...
    def compute_projected_area_sqm(self, multi_poly: MultiPolygon, source_epsg: int = 4326) -> float:
        """
        Reprojects Shapely geometry to appropriate UTM Zone (EPSG:32644 or EPSG:32645) to compute exact metric area in m².
        """
        if multi_poly.is_empty:
            return 0.0

        # Choose UTM 44N for Western Nepal (< 84°E) or UTM 45N (>= 84°E)
        target_epsg = self.TARGET_UTM_WEST_EPSG if multi_poly.centroid.x < 84.0 else self.TARGET_UTM_EPSG
        ...
        try:
            ...
            logger.warning(f"pyproj reprojection to EPSG:{target_epsg} failed ({e}). Approximating area using ellipsoidal metric scale.")
    ...
    def vectorize_mask(
        ...
        and computes projected planar area in dynamically selected UTM Zone.
        """
